...
print("="*65)
print(f"[⚡ NEXUS] Escaneando en profundidad: {RUTA_ESCANEO}\n")
        ...
        if file_hash:
            if file_hash in hashes_verificados:
                ...
                try:
                    ...
                    print(f" [! NEXUS -> ADVERTENCIA] CLON AISLADO: {visual}")
                except Exception as e:
                    print(f" [! NEXUS -> ERROR] ERROR aislando {visual}: {e}")
            else:
                hashes_verificados[file_hash] = ruta

print("\n" + "-" * 65)
if duplicados > 0:
    print(f"[✔ NEXUS -> COMPLETADO] Se aislaron {duplicados} archivos duplicados en la carpeta 'DUPLICADOS_A_BORRAR'.")
    print("[I] Revisa la carpeta y elimínala manualmente cuando estés seguro.")
else:
    print("[✔ NEXUS -> COMPLETADO] Sistema limpio. No se encontraron clones exactos.")