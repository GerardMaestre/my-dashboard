...
function getFileIconFromPath(filePath) {
	...
	if (lower.endsWith('.exe')) return '­ƒº®';
	if (lower.endsWith('.pdf')) return '­ƒôò';
	if (lower.endsWith('.zip') || lower.endsWith('.rar') || lower.endsWith('.7z')) return '­ƒù£´©Å';
	if (lower.endsWith('.mp4') || lower.endsWith('.mkv') || lower.endsWith('.avi')) return '­ƒÄ¼';
	if (lower.endsWith('.mp3') || lower.endsWith('.wav') || lower.endsWith('.flac')) return '­ƒÄÁ';
	if (lower.endsWith('.png') || lower.endsWith('.jpg') || lower.endsWith('.jpeg') || lower.endsWith('.webp')) return '­ƒû╝´©Å';
	if (lower.endsWith('.doc') || lower.endsWith('.docx')) return '­ƒôØ';
	if (lower.endsWith('.xls') || lower.endsWith('.xlsx') || lower.endsWith('.csv')) return '­ƒôè';
	if (lower.endsWith('.js') || lower.endsWith('.ts') || lower.endsWith('.py') || lower.endsWith('.bat')) return '­ƒÆ╗';
	return '­ƒôä';
}

function getAppIcon(name) {
	const n = String(name || '').toLowerCase();
	if (n.includes('chrome')) return '­ƒîÉ';
	if (n.includes('firefox')) return '­ƒªè';
	if (n.includes('edge')) return '­ƒöÀ';
	if (n.includes('discord')) return '­ƒÄ«';
	if (n.includes('spotify')) return '­ƒÄº';
	if (n.includes('steam')) return '­ƒÄ▓';
	if (n.includes('nvidia')) return '­ƒƒ®';
	if (n.includes('adobe')) return '­ƒà░´©Å';
	if (n.includes('microsoft')) return '­ƒ¬ƒ';
	if (n.includes('visual studio') || n.includes('vscode')) return '­ƒºá';
	if (n.includes('java')) return 'Ôÿò';
	if (n.includes('python')) return '­ƒÉì';
	return '­ƒº®';
}
...
async function cargarScripts() {
	...
	for (const file of validFiles) {
		...
		const folder = parts.length > 1 ? parts.slice(0, -1).join('/') : 'Miscel├íneo';

		if (favoritesList.includes(file)) {
			if (!groups['Ôÿà Destacados']) groups['Ôÿà Destacados'] = [];
			groups['Ôÿà Destacados'].push(file);
		} else {
	...
	// fragment ya est├í declarado en la l├¡nea 70

	for (const folder of sortedFolders) {
		const header = document.createElement('li');
		header.className = 'category-header';
		// No usar innerHTML con datos din├ímicos del disco (XSS en nombres de carpetas)
		...
		for (const file of groups[folder]) {
			...
			let desc = "A├▒ade 'DESC: tu descripci├│n' dentro del c├│digo de tu script para que aparezca aqu├¡.";
			...
			strong.textContent = 'Par├ímetros:';
...
function ejecutar(fileName, isAuto = false, isSilent = false) {
	...
	if (!isSilent) {
		logTerminal(`\nÔûÂ Ejecutando: ${fileName} ${args}`, 'command');
	...
	if (isExternal) {
		...
		if (!isSilent) {
			if (result && result.forcedExternal) {
				mostrarToast('Herramienta Pro ejecutada en modo visual externo autom├íticamente.', 'system');
			}
	...
	if (api.isRunning(fileName)) {
		if (!isSilent) logTerminal(`[!] Ya est├í en ejecuci├│n: ${fileName}`, 'error');
		return;
	}
...
function iniciarAutopilot() {
	...
	if (!timeInput || timeInput <= 0) {
		mostrarToast("Por favor, introduce un tiempo v├ílido mayor a 0.", "error");
		return;
	}
...
function logTerminal(mensaje, tipo) {
	...
	// Limpiar c├│digos ANSI de la consola de Python/CMD
...
api.onProcessExit(({ fileName, code }) => {
	...
	if (!silentRuns.has(fileName)) {
		...
		logTerminal(`[Fin] C├│digo ${code}`, isSuccess ? 'system' : 'error');
...
document.addEventListener('keydown', (e) => {
	...
	if (e.ctrlKey && e.key === 'l') {
		e.preventDefault();
		terminal.innerHTML = '<span class="log-system">´ú┐ Sistema Nexus inicializado. Listo para operar.</span>';
	}
...
document.getElementById('btn-clear').addEventListener('click', () => {
	terminal.innerHTML = '<span class="log-system">´ú┐ Log limpiado.</span>';
});
...
// =======================================================================
// 1. FUNCI├ôN PRINCIPAL: RENDERIZAR ESCANEO (Limpia y sin duplicados)
// =======================================================================
function renderEscaneoDisco(payload) {
	...
	if (treemap) {
        ...
        // Si el DOM colapsa por Flexbox, aseguramos un tama├▒o m├¡nimo gigante
        ...
        // P├¡xeles forzados (Ignoramos bugs de CSS porcentual)
        ...
        if (mapItems.length === 0) {
            rootContainer.innerHTML = '<div style="color:red; padding: 20px;">Error: No hay items v├ílidos para dibujar el Treemap.</div>';
        } else {
...
// =======================================================================
// 2. MOTOR MATEM├üTICO: SQUARIFY (Blindado contra divisiones por cero)
// =======================================================================
...
// =======================================================================
// 3. INYECTOR DOM: Estilo macOS (Anidado en P├¡xeles Absolutos)
// =======================================================================
...
function renderAppsGrid(apps) {
	...
	apps.slice(0, 300).forEach((app) => {
		...
		// Iniciar carga as├¡ncrona del icono real nativo al encolar la carta
...
function aplicarFiltros() {
	...
	if (!found) {
		if (!noResults) {
			...
			noResults.innerHTML = `<h3>No hay scripts para mostrar</h3><p style="opacity:0.7; margin-top:10px;">Prueba ajustando los filtros o tu b├║squeda.</p>`;
...
function getSquarifiedLayout(items, x, y, width, height, depth = 1, parentHue = null) {
	...
	function layoutRow(row, w, b) {
		...
		for (let node of row) {
			...
			// Asignamos color basado en la extensi├│n si es archivo (Estilo WizTree)
			let hue = parentHue;
			if (!isParent) {
