...
if (!gotTheLock) {
  ...
  const wireWindowLifecycle = (win) => {
    ...
    // Interceptar el cierre nativo (Alt+F4 o botón X) para mandar a bandeja
    win.on('close', (event) => {
      if (!isQuitting) {
  ...
  const registerIpcHandlers = () => {
    ...
    ipcMain.on('window-control', (event, action) => {
      ...
      if (action === 'close') {
        // En vez de cerrar, ocultamos (Tray mode)
        win.hide();
      }
  ...
  async function parseWizTreeCSV(filePath, normRoot, sender) {
    ...
    for await (const line of rl) {
      ...
      if (fullPath.toLowerCase().startsWith(normRoot)) {
         ...
         // FILTRADO AGRESIVO: 
         // En profundidades mayores a 2, ignoramos archivos pequeños para ahorrar GBs de RAM
  ...
  app.on('ready', () => {
    ...
    // --- CONFIGURACIÓN DEL TRAY (Bandeja del sistema) ---
    ...
    tray.setToolTip('Horus Engine - Operando en Segundo Plano');
